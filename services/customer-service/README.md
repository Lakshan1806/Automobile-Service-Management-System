...
`services/customer-service` now focuses exclusively on payment orchestration for the Automobile Service Management System. It exposes a compact HTTP API that creates PayHere checkout sessions, renders a one-click redirect experience for existing orders, and processes asynchronous gateway notifications.
...
WebApplication1.http                -> VS Code/REST Client sample request
```
